...
In this framed context the `data` field is already length-delimited and the call is already identified by the sink address, so the wire format differs from the legacy suffix:

1. **Marker omitted**: the 16-byte `ercMarker` MUST be omitted. Its role (identifying this standard) is taken by the `0x8021` prefix defined below.
2. **Forward-parsed**: fields are read left-to-right from the start of `data`, not backwards from the end.
3. **No length prefixes**: `codesLength`, `codeRegistryChainIdLength`, and `cborLength` MUST be omitted. The final variable-length field in each schema runs to the end of `data`, whose length is known from the call structure.
...
The schema IDs and their semantic fields are unchanged from the legacy suffix; only the framing differs:

| Schema | Legacy suffix fields | EIP-8130 `schemaData` (forward, no lengths) |
| --- | --- | --- |
| 0 | `codes \|\| codesLength \|\| schemaId \|\| ercMarker` | `codes` (to end of `data`) |
| 1 | `codes \|\| codesLength \|\| chainId \|\| chainIdLength \|\| registryAddr \|\| schemaId \|\| ercMarker` | `registryAddr (20 bytes) \|\| chainId (8 bytes, uint64 big-endian) \|\| codes (to end)` |
| 2 | `cborData \|\| cborLength \|\| schemaId \|\| ercMarker` | `cborData` (self-delimiting, to end of `data`) |

Parsers for EIP-8130 transactions MUST use this framed encoding. The legacy reverse-parsed suffix format applies only to non-EIP-8130 transaction types.
...
```
Legacy suffix:  "baseapp"(7) + codesLength(1) + schemaId(1) + ercMarker(16) = 25 bytes
EIP-8130 data:  0x8021(2)   + schemaId(1)    + "baseapp"(7)                = 10 bytes
```
