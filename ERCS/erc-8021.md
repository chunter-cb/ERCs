...
EIP-8130 extends schema 2 with additional keys for phase scoping and off-chain commitments, and allows the `cborData` to be either a single CBOR map or a CBOR array of maps for multi-record use. See the [EIP-8130 Integration](#eip-8130-integration) section.
...
### [EIP-8130](./eip-8130.md) Integration

[EIP-8130](./eip-8130.md) transactions carry no `tx.input` blob. Instead, execution is structured as a `calls` array of phases and a dedicated top-level `metadata` field carries signed, non-executing bytes. This standard uses the `metadata` field as the home for attribution data on EIP-8130 transactions.

Because the `metadata` field is already length-delimited by RLP and identified by its position in the transaction, the legacy transport overhead is dropped entirely:

- **No `ercMarker`**: the 16-byte marker is omitted; the field itself identifies the content.
- **No `0x8021` prefix**: unnecessary; parsers already know they are reading EIP-8130 attribution.
- **No length prefixes**: `codesLength`, `codeRegistryChainIdLength`, and `cborLength` are omitted. Variable-length fields run to the end of `schemaData`, whose length is known from the RLP field.
- **Forward-parsed**: fields are read left-to-right.

The `metadata` field encoding for this standard is:

```
metadata = schemaId (1 byte) || schemaData
```

#### Schema 2 — EIP-8130 native

`schemaId = 0x02`. `schemaData` is a CBOR value using deterministic encoding ([RFC 8949 §4.2](https://www.rfc-editor.org/rfc/rfc8949#section-4.2)). The `cborLength` prefix used in legacy calldata is omitted.

`schemaData` MAY be either a **CBOR map** (single record) or a **CBOR array of maps** (multi-record). Parsers distinguish by the CBOR major type of the first byte: `0xA0–0xBF` is a map; `0x80–0x9F` or `0x9F` is an array. This avoids a new schema ID.

All existing keys (`a`, `w`, `s`, `r`, `m`) remain valid. Three keys are added for EIP-8130:

| Key | Type | Description |
| --- | --- | --- |
| `a` | string | Application code |
| `w` | string | Wallet code |
| `s` | string[] | Service codes (block builders, relayers, solvers) |
| `r` | map | Custom registries per entity type |
| `m` | map | Arbitrary application metadata (key-value pairs) |
| `p` | uint | **Phase scope** (0-based index into `calls`). Absent = whole transaction. |
| `c` | uint | **Call scope** within phase `p` (0-based). Only valid when `p` is present. |
| `h` | bytes | **Off-chain commitment**: a 32-byte digest of an off-chain document, optionally prefixed with a 4-byte type indicator (`indicator (4 bytes) \|\| digest (32 bytes)` = 36 bytes). The off-chain document is self-describing; only the digest appears on-chain. |

All keys remain optional. A map with only `m` and optional scope keys carries opaque non-attribution metadata with no attribution requirement.
...
type Schema2Record = {
  ...
  p?: number;
  c?: number;
  h?: Hex; // 32 or 36 bytes
};

// metadata field is either a single record or an array of records
type Schema2Data = Schema2Record | Schema2Record[];
```

Use the array form when multiple parties contribute independent records, or when a single transaction needs records with different scopes.

#### Byte comparison

Single `"baseapp"` attribution, schema 2 (single map):

```
Legacy calldata suffix:  cborData(11) + cborLength(2) + schemaId(1) + ercMarker(16) = 30 bytes
EIP-8130 metadata:       schemaId(1)  + CBOR map(22)                               = 23 bytes
```

Single 32-byte off-chain commitment (`h` key only):

```
metadata = 0x02 || CBOR map {h: <32B digest>}
         = 1 + 37 = 38 bytes
```

Multi-party batch: attribution (whole-tx) + two per-phase memos + one commitment, schema 2 (array):

```
metadata = 0x02 || CBOR array [
  {a:"baseapp", w:"mywallet"},        // whole-tx attribution
  {m:{memo:"Invoice #123"}, p:1},     // memo scoped to phase 1
  {m:{memo:"Invoice #456"}, p:2},     // memo scoped to phase 2
  {h:<32B digest>}                    // commitment, whole-tx
]
= 112 bytes total in metadata field
```

#### Parsing algorithm (EIP-8130)

1. Read `schemaId` (first byte of `metadata`).
2. If `schemaId` is `0x02`:
   a. Read the next byte. If it is a CBOR map major type (`0xA0–0xBF`), decode as a single schema 2 record.
   b. If it is a CBOR array major type (`0x80–0x9F` or `0x9F`), decode as an array; process each element as a schema 2 record.
   c. For each record, resolve `p`/`c` scope against the `calls` array.
3. Otherwise: unknown `schemaId`; treat `metadata` as opaque.

Legacy schemas 0 and 1 are not used in EIP-8130 `metadata` because they cannot express scope or commitments; use schema 2 instead.

#### wallet_sendCalls integration

Apps pass attribution bytes as the `dataSuffix` capability in [ERC-5792](./eip-5792.md) `wallet_sendCalls`. On EIP-8130, the wallet:

1. Collects `dataSuffix` bytes from the app and its own wallet code.
2. Encodes a schema 2 or 3 record in `metadata` **after** finalising the `calls` structure (so phase indices are stable).
3. Signs `sender_auth` over the RLP including `metadata`; the payer co-signs via `payer_auth`.
...
All schemas after schema 2, when used as a legacy calldata suffix, MUST follow the structure: `{schemaData}{schemaDataLength (2 bytes)}{schemaId (1 byte)}{ercSuffix (16 bytes)}`. This layout ensures the complete suffix for any future schema can be deterministically discovered using the same parsing rules.

The array form of schema 2 is defined only for [EIP-8130](./eip-8130.md) `metadata` field usage and does not have a legacy calldata suffix form.
