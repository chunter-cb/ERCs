...
EIP-8130 defines a leaner, marker-less wire format for schema 2 in the transaction `metadata` field. See the [EIP-8130 Integration](#eip-8130-integration) section.
...
The key schema is unchanged from the legacy schema 2 (`a`, `w`, `s`, `r`, `m`); only the framing differs. A map with only `m` carries opaque non-attribution metadata with no attribution requirement.
...
> Per-phase and per-call scoping, off-chain commitment records, and multi-record encoding for independent contributors are intentionally left out of this revision and can be added later without a new schema ID (for example, by adding reserved keys or allowing `schemaData` to also be a CBOR array of maps).
