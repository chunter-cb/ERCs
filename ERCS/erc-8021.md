...
EIP-8130 extends schema 2 with an off-chain commitment key (`h`) and defines a leaner, marker-less wire format for the transaction `metadata` field. See the [EIP-8130 Integration](#eip-8130-integration) section.
...
`schemaId = 0x02`. `schemaData` is a CBOR map using deterministic encoding ([RFC 8949 §4.2](https://www.rfc-editor.org/rfc/rfc8949#section-4.2)). The `cborLength` prefix used in legacy calldata is omitted.

All existing keys (`a`, `w`, `s`, `r`, `m`) remain valid. One key is added for EIP-8130:
...
All keys remain optional. A map with only `m` carries opaque non-attribution metadata with no attribution requirement; a map with only `h` carries a bare off-chain commitment.
...
Single `"baseapp"` attribution, schema 2:
...
2. If `schemaId` is `0x02`: decode the remaining bytes as a CBOR map per schema 2.
3. Otherwise: unknown `schemaId`; treat `metadata` as opaque.

Legacy schemas 0 and 1 are not used in EIP-8130 `metadata`; use schema 2 instead.

> Per-phase and per-call scoping, and multi-record encoding for independent contributors, are intentionally left out of this revision and can be added later without a new schema ID (for example, by allowing `schemaData` to also be a CBOR array of maps).
...
2. Encodes a schema 2 map in `metadata` after finalising the `calls` structure.
...
The marker-less schema 2 wire format is defined only for [EIP-8130](./eip-8130.md) `metadata` field usage and does not have a legacy calldata suffix form.
