...
[EIP-8130](./eip-8130.md) transactions carry no `tx.input` blob; instead, execution is structured as a `calls` array of phases. Attribution is carried in a dedicated **metadata call** as defined by the EIP-8130 metadata standard, whose `to` equals the reserved metadata sink address and whose `data` holds the attribution payload.

Two payload formats are valid inside a metadata call's `data`:

**Format A — legacy passthrough**: the complete legacy suffix (including the trailing `ercMarker`) carried as-is. This allows producers that already hold a pre-built suffix (for example from a `dataSuffix` field in a viem call or a `wallet_sendCalls` capability) to place it directly into the metadata call without reformatting. Parsers detect this format by checking whether the last 16 bytes of `data` equal the `ercMarker`; if so, the entire payload is parsed backwards per the [Parsing Algorithm](#parsing-algorithm).

**Format B — native framed encoding**: a leaner forward-parsed form that omits the marker and length prefixes. The `data` field is already length-delimited by the call structure, so the marker and all length bytes are redundant. Producers generating a fresh suffix for an EIP-8130 transaction SHOULD prefer this format.

```
data = 0x8021 (2 bytes) || schemaId (1 byte) || schemaData
```

The schema IDs and their semantic fields are unchanged; only the framing differs:

| Schema | Format B `schemaData` (forward, no length prefixes) |
| --- | --- |
| 0 | `codes` (to end of `data`) |
| 1 | `registryAddr (20 bytes)` `\|\|` `chainId (8 bytes, uint64 big-endian)` `\|\|` `codes` (to end) |
| 2 | `cborData` (self-delimiting CBOR, to end of `data`) |

Parsers for EIP-8130 transactions MUST support both formats. Detection order: check the last 16 bytes for the `ercMarker` first (Format A); if not present, check the leading 2 bytes for `0x8021` (Format B); otherwise treat `data` as opaque.

Each party (application, wallet, or other service) emits its own independent metadata call. No merging of codes into a shared suffix is required or expected.

#### Byte comparison (Format B)

Single code `"baseapp"` (7 bytes), schema 0:

```
Legacy / Format A:  "baseapp"(7) + codesLength(1) + schemaId(1) + ercMarker(16) = 25 bytes
Format B:           0x8021(2)   + schemaId(1)    + "baseapp"(7)                = 10 bytes
```
