...
**Separate singleton over a Keystore method.** The Keystore is a pure authorization oracle and is deliberately signing-domain-agnostic. Housing the profile in a distinct immutable contract keeps that separation and makes the profile supersedable: a future signing standard deploys a successor verifier at a new address while the Keystore and all account state remain untouched.
